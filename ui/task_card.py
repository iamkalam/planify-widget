...
class TaskCard(Gtk.Box):
    ...
    def build_ui(self):
        ...
        self.checkbox.add_css_class("task-checkbox")
        ...
        details_row.add_css_class("task-meta-row")
        ...
        if self.due_date:
            try:
                ...
                if date_text:
                    ...
                    if self.is_overdue():
                        date_box.add_css_class("task-overdue")
                    elif date_text == "Today":
                        date_box.add_css_class("task-due-today")
                    elif date_text == "Tomorrow":
                        date_box.add_css_class("task-due-tomorrow")

                    details_row.append(date_box)
        ...
        if self.project_name and self.project_name.strip():
            try:
                ...
                details_row.append(project_box)
            except:
                pass

        content_box.append(details_row)

        # Action buttons — fade in on card hover (see .card-actions in CSS)
    ...
    def create_priority_badge(self):
        """Create a theme-aware priority dot (colors adapt to light/dark)"""
        badge = Gtk.Label(label="●")
        badge.add_css_class("priority-dot")

        try:
            if self.priority == 3:
                badge.add_css_class("priority-dot-high")
                badge.set_tooltip_text("High Priority")
            elif self.priority == 2:
                badge.add_css_class("priority-dot-medium")
                badge.set_tooltip_text("Medium Priority")
            else:
                badge.add_css_class("priority-dot-low")
